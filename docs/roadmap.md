...
- [ ] P14.4: Media deeper control — **volume scrub done**: NowPlayingBar now exposes a volume slider (0-100 %) driven by HA `volume_level` attribute; `volume_set` dispatched through DeviceCommand; percentage badge + speaker/mute icon. Queue/playlist/shuffle/repeat still TODO. Ref: home-assistant/android media controls
...
- [ ] P14.8: Power/thermal profile — **battery-saver done**: BatteryMonitor @Singleton exposes status StateFlow (level, isCharging, isLow at ≤20% unplugged); BATTERY_SAVER_ENABLED preference + Settings toggle; VoiceService.startWakeWord() skips when saver active and battery is low. Thermal throttle detection + idle wattage target still TODO
